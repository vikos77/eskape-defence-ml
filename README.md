...
| Q3b: Multi-block genomic content comparison | Five feature blocks tested (defence, IS elements, HMRG, ARG, anti-defence) alone and combined on 309 dereplicated genomes (K=6 forced, spec_score ≥ 0.70 marker filter applied to all blocks). IS element family composition (19 families after filtering) adds genuine clustering signal: Defence-only ARI = 0.219 [0.059–0.320]; IS-only ARI = 0.237 [0.150–0.341]; Defence+IS ARI = 0.392 [0.205–0.473] (gain +0.172). Anti-defence (37 types after filtering) added no meaningful signal (ARI = 0.115 [0.053–0.236]). All conditions p ≤ 0.001 (1,000-permutation test). |
