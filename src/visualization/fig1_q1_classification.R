# Fig 1: Q1 species classification — all panels composed natively in R
# Requires: results/fig1a_cm_for_r.csv
#           results/fig1a_stats_for_r.json
#           results/fig1b_holdout_for_r.json
# Output:   results/figures/final/fig1_q1_classification.{png,pdf}
...
p_a <- ggplot(cm_long, aes(x = pred_sp, y = true_sp, fill = prop)) +
...
# ── Panel B: Per-species holdout recall — vertical dot chart ──────────────────
recall_df <- data.frame(
  species = factor(SP_ORDER, levels = SP_ORDER),
  recall  = unlist(d$recalls[SP_KEYS])
)

p_b <- ggplot(recall_df, aes(x = species, y = recall)) +
  ...
  scale_y_continuous(limits = c(0.78, 1.10),
...
# ── Panel C: Four-classifier comparison ───────────────────────────────────────
# Values from q1_mcnemar_367.json fold BAs + bootstrap CIs (Table 1)
# RF at top (primary, highlighted); others ordered by BA descending below
model_df <- data.frame(
  model   = factor(
...
p_c <- ggplot(model_df, aes(x = ba, y = model)) +
  ...
  scale_fill_manual(values = c("TRUE" = "#0072B2", "FALSE" = "white"),
                    guide = "none") +
  scale_x_continuous(limits = c(0.82, 1.00),
  ...
  annotate("text", x = 0.823, y = 0.42,
           ...
           size = 2.5, hjust = 0, color = "grey55", fontface = "italic") +
...
# ── Compose ───────────────────────────────────────────────────────────────────
fig <- p_a / (p_b + p_c + plot_layout(widths = c(1, 1))) +
...
save_fig(fig, "results/figures/final/fig1_q1_classification", width = 7.5, height = 8.5)
