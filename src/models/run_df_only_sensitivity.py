"""
Sensitivity analysis: DefenseFinder-only feature matrix.

Removes 13 PADLOC-specific (dp_padloc_*) features from the 236-feature
named matrix, leaving 223 DF-specific (dp_df_*) + shared (dp_*) features.
Motivation: matches the primary tool used in the published Acinetobacter
analysis (Muthuraman et al. 2026, JAM).

Runs Q1 (species classification) and Q2 (ARG-burden prediction) with
identical protocol to run_q1_named.py and run_q2_named.py.

Outputs:
  results/sensitivity_df_only_q1.json
  results/sensitivity_df_only_q2.json
"""
...
df_only_cols = [c for c in all_dp if not c.startswith("dp_padloc_")]
print(f"Named dp_ features:        {len(all_dp)}")
print(f"PADLOC-specific removed:   {len(all_dp) - len(df_only_cols)}")
print(f"DF-only feature pool:      {len(df_only_cols)}")

# ── Q1 ───────────────────────────────────────────────────────────────────────
print("\n" + "="*55)
print("Q1: DefenseFinder-only")

species_list = sorted(fm["species"].unique())
sp_prev = pd.DataFrame({sp: fm[fm.species==sp][df_only_cols].mean()
                        for sp in species_list})
spec_score = sp_prev.std(axis=1) / 0.5
markers    = spec_score[spec_score >= SPEC_THRESH].index.tolist()
FEAT_COLS  = [c for c in df_only_cols if c not in markers]

print(f"Taxonomic markers removed (spec>={SPEC_THRESH}): {len(markers)} -> {markers}")
...
q1_results = {
    "n_features_pool"  : len(df_only_cols),
...
for sp in species_list:
    ...
    feat_prev = q2_df[df_only_cols].mean()
...
# ── Save ─────────────────────────────────────────────────────────────────────
with open("results/sensitivity_df_only_q1.json", "w") as f:
    json.dump(q1_results, f, indent=2)
with open("results/sensitivity_df_only_q2.json", "w") as f:
    json.dump(q2_results, f, indent=2)

print("\nSaved: results/sensitivity_df_only_q1.json")
print("Saved: results/sensitivity_df_only_q2.json")
print("Done.")
