"""
Q4 SHAP interpretation — named-236 feature matrix.

Fold-averaged SHAP: for each of 5 GroupedStratifiedKFold folds, train a fresh
RF (named best params) on the training partition, compute SHAP on the held-out
test partition, reassemble into (3335, 231, 6). Every genome's SHAP values come
from a model that never saw it in training.

Outputs:
  results/q4_shap_named_global.csv     — feature, global_shap, global_rank
  results/q4_shap_named_perclass.csv   — feature, species, mean_abs_shap, rank
  results/q4_shap_named_array.npy      — raw (3335, 231, 6) SHAP array

Prints:
  - Global top-20 features
  - Per-class top-10 for all 6 species
  - Alignment table: published Fisher's exact systems vs named-236 SHAP ranks
...
fm      = pd.read_parquet(PROC / "feature_matrix_3335_named.parquet")
...
with open(RES / "q1_named_results.json") as f:
    _res = json.load(f)
...
# ── Save raw array ────────────────────────────────────────────────────────────
np.save(RES / "q4_shap_named_array.npy", shap_3d)
print(f"\nSaved: {RES / 'q4_shap_named_array.npy'}  shape={shap_3d.shape}")
...
global_df.to_csv(RES / "q4_shap_named_global.csv", index=False)
print(f"Saved: {RES / 'q4_shap_named_global.csv'}")
...
perclass_df.to_csv(RES / "q4_shap_named_perclass.csv", index=False)
print(f"Saved: {RES / 'q4_shap_named_perclass.csv'}")

# ── Print global top-20 ───────────────────────────────────────────────────────
print()
print("=" * 68)
print("GLOBAL TOP-20 FEATURES  (mean |SHAP| over 3335 genomes x 6 classes)")
print(f"{'Rank':>4}  {'Feature':<35}  {'Mean |SHAP|':>11}")
print("-" * 68)
for _, row in global_df.head(20).iterrows():
    print(f"  {int(row['global_rank']):>2}  {row['feature']:<35}  {row['global_shap']:.5f}")
...
for sp in SPECIES_ORDER:
    ...
    for _, row in sub.iterrows():
        print(f"  {int(row['rank']):>4}  {row['feature']:<35}  {row['mean_abs_shap']:.5f}")

# ── Alignment table: published Fisher's exact vs named-236 SHAP ───────────────
PUBLISHED_TO_DP = {
...
print()
print("=" * 75)
print("ALIGNMENT TABLE  — published Fisher's exact systems vs named-236 SHAP")
...
for pub_sys, dp_feats in PUBLISHED_TO_DP.items():
    ...
    if not present:
        print(f"  {pub_sys:<14}  {'(not in named-236 FEAT_COLS)':<35}  {'—':>11}  {'—':>7}")
        continue
